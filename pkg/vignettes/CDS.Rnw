...
J.P. Morgan including Blythe Masters.\cite{cdsOrigins,
...
cetera \cite{jk}. In the \textbf{CDS} package, we focus on calculations
...
\item [Trade Date] or \textbf{TDate} as shown in the argument of the
  \texttt{CDS} function. It indicates the trade date of the CDS
  contract and is April 15, 2014 in \texttt{cds1}.
...
A market participant can also update the \textt{CDS} class objects she
...
\texttt{cds3} is a new ``CDS'' class object with a spread of 55
...
\begin{equation}
  \text{Default Prob} \approx (1 - exp(\frac{rt}{1-R})),
\end{equation}
