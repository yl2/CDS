...
the protection seller. As shown in Figure \ref{fig:cashFlow},
the buyer pays a stream of coupon payments, called the premium leg in
order to receive a one-off, contingent payment (protection leg) in the
case of a credit event.\\

%% add a diagram on buyer and seller of CDS?

\begin{figure}
  \caption{\label{fig:cashFlow} Mechanics of a CDS}
\begin{center}
\begin{tikzpicture}[
