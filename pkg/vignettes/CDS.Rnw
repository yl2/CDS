...
\item[Maturity] refers to the fixed date on which a CDS contract terminates. 
  But, speaking colloquially, it can also be the length of time remaining until
  that date. This length is also called the \textbf{tenor} of a CDS
  contract. The most commonly traded CDS are the 5 years. The
...
\item[Notional] is the amount of the underlying asset on which the
  payments are based.
\item[Coupon] is quoted in basis points. It specifies the
  payment amount from the protection buyer to the seller on an annual
  basis. Coupons are paid quarterly.
\item[Spread] is quoted in basis points per year. If, instead of using
  ...
  protection were to agree on a variable coupon, then the spread is the
  coupon size that they would agree on.

\item[Credit Event] triggers the settlement under a CDS
  contract. Possible credit events include banruptcy and debt restructing.
\end{description}
...
In April 2009 in North America, ISDA
...
specifications and pricing details of the Standard Model.
...
\item [Standard Coupon] is either 100 or 500 bps per
  year for CDS contracts in North America, and 25, 100, 500 or 1000 bps in Europe.
\item [Recovery Rate] is the estimated percentage of par value that
  ...
  reported in percentages of notional value. CDS contracts for corporate bonds
