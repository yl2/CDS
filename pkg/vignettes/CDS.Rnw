...
the contract. \texttt{couponRate} is the fixed coupon of the
...
does not have to know the information to use the package.

Calling \texttt{summary} on the ``CDS'' class object \texttt{cds1}
provides some essential information on the contract (including trade
date, maturity date, spread, and coupon rate as specified by the user)
and some relevant calculations based on the input CDS contract.
