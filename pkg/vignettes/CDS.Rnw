...
one-off, contingent payment (notional less recovery amount after a
credit event), called the protection leg, in the case of a credit
event.\\
...
As reflected in the example, a CDS contract between two counterparties
...
specified by the user) and relevant calculations based on the input
CDS contract and the ISDA conventions.
...
This section provides some key specifications of the ISDA Standard Model. \footnote{Refer to the \textit{ISDA Standard CDS Converter Specification} for details.}
